...
function formatLengthAlwaysFeet(val) {
    ...
    const res = `${feet}'-${inches}${fractionStr}"`;
    return res.replace(/1\/2/g, '\xBD')
              .replace(/3\/4/g, '\xBE')
              .replace(/1\/4/g, '\xBC');
}
...
function getResolvedPanelProperties(panel, style) {
    ...
    if (panel.basePlateConfig) {
        if (panel.basePlateConfig.plateShape === 'qiw_standard') {
            const isQBP54 = panel.basePlateConfig.qiwPlateType === 'QBP54';
            bpW = isQBP54 ? 5.0 : 4.0;
            bpL = isQBP54 ? 5.0 : 4.0;
            ...
            if (panel.basePlateConfig.width !== undefined) bpW = parseFloat(panel.basePlateConfig.width) || 6.0;
            if (panel.basePlateConfig.height !== undefined) bpL = parseFloat(panel.basePlateConfig.height) || 6.0;
            if (panel.basePlateConfig.thickness !== undefined) bpH = parseFloat(panel.basePlateConfig.thickness) || 0.5;
            if (panel.basePlateConfig.corners && panel.basePlateConfig.corners.diameter !== undefined) {
                bpHoleD = parseFloat(panel.basePlateConfig.corners.diameter) || 0.5;
                bpHoleOffsetX = parseFloat(panel.basePlateConfig.corners.margin) || 0.5;
                bpHoleOffsetY = parseFloat(panel.basePlateConfig.corners.margin) || 0.25;
            }
            if (panel.basePlateConfig.grid && panel.basePlateConfig.grid.diameter !== undefined) {
                bpHoleD = parseFloat(panel.basePlateConfig.grid.diameter) || 0.5;
            }
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function getDefaultBasePlateConfig() {
        return {
            ...
            custom: [
                { id: 1, x: "1.0", y: "1.0", diameter: "0.5" },
                { id: 2, x: "5.0", y: "1.0", diameter: "0.5" },
                { id: 3, x: "5.0", y: "5.0", diameter: "0.5" },
                { id: 4, x: "1.0", y: "5.0", diameter: "0.5" }
            ]
        };
    }
    ...
    function getDefaultPanelConfig() {
        return {
            ...
            basePlateConfig: getDefaultBasePlateConfig(),
            meshGridW: 2.0,
            meshGridH: 2.0,
            meshWireD: 0.135,
            extraFlatBar: 'no',
    ...
    function createDefaultReturnConfigWithMainStyle(mainPanel, returnType) {
        ...
        if (mainPanel) {
            ...
            const customKeys = ['postType', 'postSize', 'postW', 'postH', 'postT', 'topRailType', 'topRailSize', 'topRailW', 'topRailH', 'topRailT', 'botRailType', 'botRailSize', 'botRailW', 'botRailH', 'botRailT', 'midRailType', 'midRailSize', 'midRailW', 'midRailH', 'midRailT', 'midRailGap', 'picketType', 'picketSize', 'picketW', 'picketH', 'picketT', 'picketSpacing', 'includeBasePlates', 'basePlateConfig'];
            customKeys.forEach(k => {
                if (mainPanel[k] !== undefined) {
                    if (k === 'basePlateConfig' && mainPanel[k]) {
                        cfg[k] = JSON.parse(JSON.stringify(mainPanel[k]));
                    } else {
                        cfg[k] = mainPanel[k];
                    }
                }
    ...
    function updateInputs() {
        ...
        if (cat === 'rail_catalog') {
            ...
            if (!panelObj) {
                ...
                // Base Plates Checkbox and Link
                html += `
                <div class="input-group" style="grid-column: span 2; display: flex; align-items: center; gap: 8px; margin-top: 10px; margin-bottom: 10px;">
                    <input type="checkbox" id="inp-includeBasePlates" style="width: 16px; height: 16px; cursor: pointer;">
                    <label for="inp-includeBasePlates" style="margin: 0; cursor: pointer; display: flex; align-items: center; gap: 4px; font-weight: 500; font-size: 13px; color: var(--text-color);">
                        <span>Include</span>
                        <span id="lnk-editBasePlate" style="color: var(--primary-color, #3FC8BB); text-decoration: underline; font-weight: bold; cursor: pointer;">Base Plate</span>
                    </label>
                </div>
                `;
            ...
            const editBPLnk = document.getElementById('lnk-editBasePlate');
            if (editBPLnk) {
                editBPLnk.addEventListener('click', (e) => {
                    e.preventDefault();
                    saveCurrentInputsToActivePanel();
                    balconyWizardState.activeMode = 'basePlate';
                    updateInputs();
                    if (pdfPreviewModeActive) {
                        svgContainer.classList.add('hidden');
                        pdfIframe.classList.remove('hidden');
    ...
    function renderCurrentCAD(isPreview = true) {
        ...
        try {
            ...
            if (svgElement) {
                // Diagnostic logger for large paths
                svgElement.querySelectorAll('path').forEach(path => {
                    try {
                        ...
                        if (bbox && bbox.width > 100 && bbox.height > 100) {
                            ...
                            const parentId = path.parentElement ? (path.parentElement.id || path.parentElement.tagName) : '';
                            fetch('/log?msg=' + encodeURIComponent('DIAG_PATH: id=' + id + ', parent=' + parentId + ', bbox=' + bbox.width + 'x' + bbox.height + ', d=' + d.substring(0, 100)));
                        }
                    } catch (e) {}
                });
                ...
                svgElement.querySelectorAll('path').forEach(path => {
                    try {
                        ...
                        if (totalLen > 100) {
                            ...
                            const pEnd = path.getPointAtLength(totalLen);
                            if (pStart && pEnd) {
                                const dx = Math.abs(pStart.x - pEnd.x);
                                const dy = Math.abs(pStart.y - pEnd.y);
                                if (dx > 100 && dy > 100) {
                                    path.remove();
                                }
                            }
    ...
    async function triggerPiecesDxfDownload() {
        ...
        if (cat === 'welded_assembly') {
            ...
            if (isLoose) {
                ...
                if (includeBasePlates === 'yes' && currentModel.models.basePlates && currentModel.models.basePlates.models.bpLoose) {
                    ...
                    const actualBpMark = resolveBasePlatePieceMark(bpc);
                    addPiece(actualBpMark, currentModel.models.basePlates.models.bpLoose);
                }
                ...
                if (bpMark && vals.includeBasePlates === 'yes' && currentModel.models.basePlates && currentModel.models.basePlates.models) {
                    ...
                    if (keys.length > 0) {
                        const bpc = vals.basePlateConfig || getDefaultBasePlateConfig();
                        const actualBpMark = resolveBasePlatePieceMark(bpc);
                        addPiece(actualBpMark, currentModel.models.basePlates.models[keys[0]]);
                    }
                }
        ...
        const getPieceDxfFilename = (mark) => (mark.toLowerCase().startsWith('bp')) ? `${mark}.dxf` : `SteelDraft_Piece_${mark}.dxf`;
        ...
        if (useZip) {
            ...
            for (const item of dxfPieces) {
                ...
                if (dxf) {
                    zip.file(getPieceDxfFilename(item.mark), dxf);
                }
            }
            ...
            try {
                ...
                for (const item of dxfPieces) {
                    ...
                    aInner.href = url;
                    aInner.download = getPieceDxfFilename(item.mark);
                    aInner.click();
                    URL.revokeObjectURL(url);
                }
            }
            ...
            for (const item of dxfPieces) {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = getPieceDxfFilename(item.mark);
                a.click();
                URL.revokeObjectURL(url);
            }
        }
    ...
    async function generateBalconyZipBatch(fabNo, jobNo, revision, finishText, needFBOM, jobName, gc, address, cityState, drawnBy, checkedBy) {
        ...
        for (const p of uniqueMasterItems) {
            ...
            try {
                ...
                if (typeof showVisualError === 'function') {
                    showVisualError("Failed to generate PDF for Master " + p.originalDrawingNo + ": " + err.message + "\n" + err.stack);
                }
            }
        }
        ...
        Object.keys(groupsBySignature).forEach(sig => {
            ...
            bomItems.forEach(item => {
                ...
                if (shapeCol.includes('PLATE') || shapeCol === 'PL') shapeCol = 'PL';
                if (shapeCol === 'FLAT_BAR' || shapeCol === 'FB') shapeCol = 'FB';
                ...
                } else if (shapeLower === 'plate' || shapeLower === 'by' || descLower.includes('plate')) {
                    categoryCol = "PLATE";
        ...
        // Gather and export flat base plate DXF files in the ZIP package
        const bpConfigsInProject = [];
        const addBpConfig = (cfg) => {
            if (!cfg) return;
            const exists = bpConfigsInProject.some(existing => areBasePlatesIdentical(existing, cfg));
            if (!exists) {
                bpConfigsInProject.push(cfg);
            }
        };

        if (balconyWizardState.sets) {
            balconyWizardState.sets.forEach(set => {
                if (set.main && set.main.includeBasePlates === 'yes') {
                    addBpConfig(set.main.basePlateConfig);
                }
                if (set.leftReturn && set.leftReturn.includeBasePlates === 'yes') {
                    addBpConfig(set.leftReturn.basePlateConfig);
                }
                if (set.rightReturn && set.rightReturn.includeBasePlates === 'yes') {
                    addBpConfig(set.rightReturn.basePlateConfig);
                }
            });
        }

        bpConfigsInProject.forEach(bpConfig => {
            const pieceMark = resolveBasePlatePieceMark(bpConfig);
            try {
                const bpModel = createCustomBasePlateModel(bpConfig);
                const dxfContent = CadEngine.exportDXF(bpModel);
                if (dxfContent) {
                    zip.file(`${pieceMark}.dxf`, dxfContent);
                }
            } catch (err) {
                console.error(`Failed to generate DXF for base plate ${pieceMark}:`, err);
            }
        });
        ...
        // Generate Parts Excel inside ZIP
        const partsList = balconyWizardState.customParts || [];
        if (window.XLSX) {
            const partsWb = XLSX.utils.book_new();
            const partsWsData = [
                [], // Row 1 is empty
                ["", "Quantity", "Sage #", "Description", "Finish", "Special order instructions"] // Row 2
            ];
            partsList.forEach(p => {
                partsWsData.push(["", p.quantity, "", p.description, p.finish, p.specialInstruction]);
            });
            ...
            partsWs['!cols'] = [
                ...
                { wch: 10 },  // B (Quantity)
                { wch: 10 },  // C (Sage #)
                { wch: 35 },  // D (Description)
                { wch: 25 },  // E (Finish)
                { wch: 35 }   // F (Special order instructions)
            ];
            XLSX.utils.book_append_sheet(partsWb, partsWs, "Parts");
            const partsExcelBuffer = XLSX.write(partsWb, { bookType: 'xlsx', type: 'array' });
            zip.file(`Parts-${jobNo}-${fabNo}.xlsx`, partsExcelBuffer);
        } else {
            let csvContent = "\n,Quantity,Sage #,Description,Finish,Special order instructions\n";
            partsList.forEach(p => {
                csvContent += `,${p.quantity},,"${(p.description || '').replace(/"/g, '""')}","${(p.finish || '').replace(/"/g, '""')}","${(p.specialInstruction || '').replace(/"/g, '""')}"\n`;
            });
            zip.file(`Parts-${jobNo}-${fabNo}.csv`, csvContent);
        }
        ...
        img.onload = function() {
            ...
            if (cat === 'rail_catalog' && !isLoosePost) {
                ...
                // Calculate properties for vertical dimensioning on the right
                const isMeshStyle = (style === 'urban_balcony' || style === 'villa_balcony' || style === 'urban_custom' || style === 'villa_custom');
                let hasFlatBar = false;
                let topOfFb = 0;
                let fbGap = 0;
                let yStart = 0;
                let yEnd = 0;
            ...
            if (cat === 'rail_catalog' && !isLoosePost) {
                ...
                let picketW = (style === 'classical' || style === 'executive') ? 0.5 : getPicketDimension(vals.picketType, vals.picketSize, vals.picketW || 0.5);
                        ...
                        // Helper to untangle intersecting leader lines
                        function untangleLeaders(leaders, labelY) {
                            ...
                            for (let iter = 0; iter < 100 && changed; iter++) {
                                ...
                                for (let i = 0; i < leaders.length; i++) {
                                    for (let j = i + 1; j < leaders.length; j++) {
                                        const l1 = leaders[i];
                                        const l2 = leaders[j];
                                        const p1_start = [l1.labelX, labelY];
                                        const p1_end = [l1.pdfTargetX, l1.pdfTargetY];
                                        const p2_start = [l2.labelX, labelY];
                                        const p2_end = [l2.pdfTargetX, l2.pdfTargetY];
                                        if (intersect(p1_start, p1_end, p2_start, p2_end)) {
                                            const tmp = l1.labelX;
                                            l1.labelX = l2.labelX;
                                            l2.labelX = tmp;
                                            changed = true;
                        ...
                        let topLabelY_forUntangle;
                        if (isMeshStyle) {
                            topLabelY_forUntangle = useCompressedDims ? (pdfY - 4.5) : (pdfY - 6.5);
                        } else {
                            topLabelY_forUntangle = useCompressedDims ? (pdfY - 24.0) : (pdfY - 32.0);
                        }
                        untangleLeaders(topLeaders, topLabelY_forUntangle);

                        topLeaders.forEach(lead => {
                            drawTopBottomLeader(lead.cx, lead.cy, lead.labelX, topLabelY_forUntangle, lead.text, lead.leaderId, true);
                        });
                        ...
                        bottomLeaders.forEach(lead => {
                            drawTopBottomLeader(lead.cx, lead.cy, lead.labelX, bottomLabelY_forUntangle, lead.text, lead.leaderId, false);
                        });
            ...
            const drawBasePlateDetail = (doc, boxX, boxY, boxW, boxH, vals) => {
                ...
                // Retrieve base plate config or construct fallback
                let bpc = vals.basePlateConfig;
                if (!bpc) {
                    bpc = {
                        plateShape: 'rect',
                        width: parseFloat(vals.basePlateW) || 6.0,
                        height: parseFloat(vals.basePlateL) || 6.0,
                        thickness: parseFloat(vals.basePlateT) || 0.5,
                        pattern: 'corners',
                        holeType: 'round',
                        corners: {
                            diameter: parseFloat(vals.basePlateHoleD) || 0.5,
                            margin: parseFloat(vals.basePlateHoleOffsetX) || 1.0
                        }
                    };
                }
                ...
                // Draw base plate plan view geometry
                doc.setLineWidth(0.2);
                const walkPlan = (m, origin) => {
                    ...
                    if (m.paths) {
                        for (const pk in m.paths) {
                            ...
                            if (path.type === 'line') {
                                const x1 = cx1 + (currentOrg[0] + path.origin[0]) * bpScale;
                                const y1 = cy1 - (currentOrg[1] + path.origin[1]) * bpScale;
                                const x2 = cx1 + (currentOrg[0] + path.end[0]) * bpScale;
                                const y2 = cy1 - (currentOrg[1] + path.end[1]) * bpScale;
                                doc.line(x1, y1, x2, y2);
                            } else if (path.type === 'circle') {
                                const x = cx1 + (currentOrg[0] + path.origin[0]) * bpScale;
                                const y = cy1 - (currentOrg[1] + path.origin[1]) * bpScale;
                                const r = path.radius * bpScale;
                                doc.circle(x, y, r, 'S');
                    ...
                    if (m.models) {
                        for (const mk in m.models) {
                            walkPlan(m.models[mk], currentOrg);
                        }
                    }
                };
                ...
                if (margins.targetHole) {
                    ...
                    const plateLeftX1 = cx1 - w/2 * bpScale;
                    const plateBotY1 = cy1 + h/2 * bpScale;
                    const hx_center1 = cx1 + (margins.targetHole.x - margins.bounds.minX - w/2) * bpScale;
                    ...
                    // Ticks
                    doc.line(plateLeftX1 - 0.3, xDimY1 - 0.3, plateLeftX1 + 0.3, xDimY1 + 0.3);
                    doc.line(hx_center1 - 0.3, xDimY1 - 0.3, hx_center1 + 0.3, xDimY1 + 0.3);
                ...
                const formatPostDim = (val) => {
                    ...
                    let totalInchesVal = feet * 12 + inches;
                    if (totalInchesVal === 0 && fractionStr !== '') {
                        return fractionStr.trim();
                    } else {
                        return `${totalInchesVal}${fractionStr}`;
                    }
                };

                const getPostGaugeSuffix = (tVal) => {
                    if (typeof tVal === 'string') {
                        let s = tVal.trim().toUpperCase();
                        if (s.includes('GA')) return s;
                        if (s.includes('/')) {
                            if (!s.endsWith('"')) s = s + '"';
                            return s;
                        }
                    ...
                    if (typeof tVal !== 'number' || isNaN(tVal)) return '11GA';
                    
                    if (Math.abs(tVal - 0.1196) < 0.015 || Math.abs(tVal - 0.12) < 0.015) {
                        return '11GA';
                    } else if (Math.abs(tVal - 0.1345) < 0.01 || Math.abs(tVal - 0.135) < 0.01) {
                        return '10GA';
                    } else if (Math.abs(tVal - 0.0598) < 0.01 || Math.abs(tVal - 0.06) < 0.01) {
                        return '16GA';
                    } else if (Math.abs(tVal - 0.0747) < 0.01 || Math.abs(tVal - 0.075) < 0.01) {
                        return '14GA';
                    } else if (Math.abs(tVal - 0.1046) < 0.01 || Math.abs(tVal - 0.1) < 0.01) {
                        return '12GA';
                    } else if (Math.abs(tVal - 0.1875) < 0.01) {
                        return '3/16"';
                    } else if (Math.abs(tVal - 0.25) < 0.01) {
                        return '1/4"';
                    } else if (Math.abs(tVal - 0.125) < 0.01) {
                        return '1/8"';
                    }
                    const frac = formatFraction(tVal);
                ...
                if (cat === 'rail_catalog') {
                    if (style === 'classical' || style === 'executive') {
                        ...
                        const props = getResolvedPanelProperties(vals, style);
                        bpPostT = props ? props.postT : (vals.postT || 0.1196);
                    }
                } else if (cat === 'rails_gates') {
                    if (vals.leftPostSize && vals.leftPostSize !== 'CUSTOM') {
                        const parts = vals.leftPostSize.replace(/HSS/gi, '').trim().split('x');
                        if (parts.length >= 3) {
                            bpPostT = parts[2];
                        } else {
                            bpPostT = vals.leftPostT || 0.1196;
                        }
                    } else {
                        bpPostT = vals.leftPostT || 0.1196;
                    }
                } else if (cat === 'fence') {
                    if (vals.postSize && vals.postSize !== 'CUSTOM') {
                        const parts = vals.postSize.replace(/HSS/gi, '').trim().split('x');
                        if (parts.length >= 3) {
                            bpPostT = parts[2];
                        } else {
                            bpPostT = vals.postT || 0.1196;
                        }
                    } else {
                        bpPostT = vals.postT || 0.1196;
                    }
                ...
                if (holes.length > 0) {
                    ...
                    const bounds = bpc.plateShape === 'rect' ? { minX: 0, minY: 0 } : getPolyBounds(bpc.polyVerts);
                    const hpx = cx1 + (brHole.x - bounds.minX - w/2) * bpScale;
                    const hpy = cy1 - (brHole.y - bounds.minY - h/2) * bpScale;
                    
                    const holeDesc = bpc.holeType === 'slotted'
            ...
            function drawCadDimension(cx1, cy1, cx2, cy2, offsetMm, text, textSide, dimId, forceVerticalX = null) {
                ...
                if (dimId && annotationOffsets[dimId] !== undefined) {
                    ...
                    } else if (Math.abs(cx1 - cx2) < 0.01 || forceVerticalX !== null) {
                ...
                if (forceVerticalX !== null) {
                    px = finalOffsetMm < 0 ? -1 : 1;
                    py = 0;
                    
                    const rx_ref = forceVerticalX * cosS;
                    const ry_ref = forceVerticalX * sinS;
                    const p_ref = cadToPdf(rx_ref, ry_ref);
                    
                    const xDim = p_ref[0] + px * Math.abs(finalOffsetMm);
                    d1 = [xDim, p1[1]];
                    d2 = [xDim, p2[1]];
                } else {
                    px = -dy / len;
                    ...
                    d1 = [p1[0] + px * finalOffsetMm, p1[1] + py * finalOffsetMm];
                    d2 = [p2[0] + px * finalOffsetMm, p2[1] + py * finalOffsetMm];
                }
            ...
            if (currentMode === 'draft') {
                ...
                if (bpMark) {
                    ...
                    bomItems.push({
                        ...
                        desc: bpDesc,
                        remark: "BASE PLATE",
                        len: formatLengthAlwaysFeet(bpL),
                        weight: Math.round(wVal * 10) / 10,
                        shape: 'PL',
                        size: bpDesc,
                        len_dec: bpL
                ...
                if (isLoosePost) {
                    ...
                    if (includeBasePlates === 'yes') {
                        ...
                        const bpDesc = isQiw 
                            ? (bpc.qiwPlateType || 'QBP43') 
                            : ("PL " + formatFraction(bpc.width || 6.0) + "x" + formatFraction(bpc.thickness || 0.5)).replace(/""/g, '"');
                    ...
                    let picketT = 0.0598;
                    let picketSpacing = 4.0;
                    let includeBasePlates = vals.includeBasePlates || 'no';
                    let bpW = 6.0;
                    let bpL = 6.0;
                    let bpH = 0.5;
                    let bpHoleD = 0.5;
                    ...
                    if (includeBasePlates === 'yes') {
                        ...
                        if (totalPostsCount > 0) {
                            ...
                            const bpH_val = isQiw ? (bpc.qiwPlateType === 'QBP54' ? 5.0 : 4.0) : (bpc.height || 6.0);
                            const bpT_val = isQiw ? (bpc.qiwPlateType === 'QBP54' ? 0.25 : 0.1875) : (bpc.thickness || 0.5);
                            const wVal = calculateWeight('plate', 'CUSTOM', bpH_val, { w: bpW_val, h: bpH_val, t: bpT_val }, totalPostsCount * assemblyQty);
            ...
            const getSteelGrade = (shapeName) => {
                ...
                let grade = 'A500';
                if (s.includes('plate') || s.includes('pl') || s.includes('fb') || s.includes('flat') || s.includes('bar')) {
                    grade = 'A36';
                } else if (s === 'wwm') {
                    grade = 'WELDED';
                }
    ...
    // ==========================================
    // --- CUSTOM BASE PLATE DESIGNER SECTION ---
    // ==========================================

    function areBasePlatesIdentical(bp1, bp2) {
        if (!bp1 || !bp2) return false;
        
        // Compare shape and thickness
        if (bp1.plateShape !== bp2.plateShape) return false;
        if (bp1.plateShape === 'qiw_standard') {
            return bp1.qiwPlateType === bp2.qiwPlateType;
        }
        
        if (Math.abs((parseFloat(bp1.thickness) || 0.5) - (parseFloat(bp2.thickness) || 0.5)) > 0.001) return false;
        
        // Compare outer boundary dimensions
        if (bp1.plateShape === 'rect') {
            if (Math.abs((parseFloat(bp1.width) || 6) - (parseFloat(bp2.width) || 6)) > 0.001) return false;
            if (Math.abs((parseFloat(bp1.height) || 6) - (parseFloat(bp2.height) || 6)) > 0.001) return false;
        } else {
            // Compare poly vertices
            const v1 = (bp1.polyVerts || []).filter(v => v.x !== "" && v.y !== "");
            const v2 = (bp2.polyVerts || []).filter(v => v.x !== "" && v.y !== "");
            if (v1.length !== v2.length) return false;
            for (let i = 0; i < v1.length; i++) {
                if (Math.abs((parseFloat(v1[i].x) || 0) - (parseFloat(v2[i].x) || 0)) > 0.001) return false;
                if (Math.abs((parseFloat(v1[i].y) || 0) - (parseFloat(v2[i].y) || 0)) > 0.001) return false;
            }
        }
        
        // Compare hole type and slot config
        if (bp1.holeType !== bp2.holeType) return false;
        if (bp1.holeType === 'slotted') {
            if (Math.abs((parseFloat(bp1.slotLength) || 1) - (parseFloat(bp2.slotLength) || 1)) > 0.001) return false;
            if (Math.abs((parseFloat(bp1.slotAngle) || 0) - (parseFloat(bp2.slotAngle) || 0)) > 0.001) return false;
        }
        
        // Compare pattern type and properties
        if (bp1.pattern !== bp2.pattern) return false;
        if (bp1.pattern === 'corners') {
            const c1_d = bp1.corners ? (parseFloat(bp1.corners.diameter) || 0.5) : 0.5;
            const c2_d = bp2.corners ? (parseFloat(bp2.corners.diameter) || 0.5) : 0.5;
            const c1_m = bp1.corners ? (parseFloat(bp1.corners.margin) || 1) : 1;
            const c2_m = bp2.corners ? (parseFloat(bp2.corners.margin) || 1) : 1;
            if (Math.abs(c1_d - c2_d) > 0.001) return false;
            if (Math.abs(c1_m - c2_m) > 0.001) return false;
        } else if (bp1.pattern === 'grid') {
            const g1 = bp1.grid || {};
            const g2 = bp2.grid || {};
            if ((parseInt(g1.rows) || 2) !== (parseInt(g2.rows) || 2)) return false;
            if ((parseInt(g1.cols) || 2) !== (parseInt(g2.cols) || 2)) return false;
            if (Math.abs((parseFloat(g1.diameter) || 0.5) - (parseFloat(g2.diameter) || 0.5)) > 0.001) return false;
            if (Math.abs((parseFloat(g1.marginX) || 1) - (parseFloat(g2.marginX) || 1)) > 0.001) return false;
            if (Math.abs((parseFloat(g1.marginY) || 1) - (parseFloat(g2.marginY) || 1)) > 0.001) return false;
        } else if (bp1.pattern === 'custom') {
            const c1 = (bp1.custom || []).filter(h => h.x !== "" && h.y !== "");
            const c2 = (bp2.custom || []).filter(h => h.x !== "" && h.y !== "");
            if (c1.length !== c2.length) return false;
            for (let i = 0; i < c1.length; i++) {
                if (Math.abs((parseFloat(c1[i].x) || 0) - (parseFloat(c2[i].x) || 0)) > 0.001) return false;
                if (Math.abs((parseFloat(c1[i].y) || 0) - (parseFloat(c2[i].y) || 0)) > 0.001) return false;
                if (Math.abs((parseFloat(c1[i].diameter) || 0.5) - (parseFloat(c2[i].diameter) || 0.5)) > 0.001) return false;
            }
        }
    ...
    function resolveBasePlatePieceMark(bpConfig) {
        ...
        const sets = balconyWizardState.sets || [];
        sets.forEach(set => {
            if (set.main && set.main.includeBasePlates === 'yes') {
                addToUnique(set.main.basePlateConfig);
            }
            if (set.leftReturn && set.leftReturn.includeBasePlates === 'yes') {
                addToUnique(set.leftReturn.basePlateConfig);
            }
            if (set.rightReturn && set.rightReturn.includeBasePlates === 'yes') {
                addToUnique(set.rightReturn.basePlateConfig);
            }
        });
        
        if (balconyWizardState.tempSet) {
            const set = balconyWizardState.tempSet;
            if (set.main && set.main.includeBasePlates === 'yes') {
                addToUnique(set.main.basePlateConfig);
            }
            if (set.leftReturn && set.leftReturn.includeBasePlates === 'yes') {
    ...
    function getPolyBounds(verts) {
        const vs = verts.filter(v => v.x !== "" && v.y !== "");
        if (!vs.length) return { minX: 0, minY: 0, maxX: 6, maxY: 6, width: 6, height: 6 };
        const xs = vs.map(v => parseFloat(v.x) || 0), ys = vs.map(v => parseFloat(v.y) || 0);
        const minX = Math.min(...xs), maxX = Math.max(...xs), minY = Math.min(...ys), maxY = Math.max(...ys);
        return { minX, minY, maxX, maxY, width: Math.max(maxX - minX, 0.001), height: Math.max(maxY - minY, 0.001) };
    }

    function ptInPoly(x, y, verts) {
        const vs = verts.filter(v => v.x !== "" && v.y !== "");
        if (vs.length < 3) return false;
        let inside = false;
        for (let i = 0, j = vs.length - 1; i < vs.length; j = i++) {
            const xi = parseFloat(vs[i].x) || 0, yi = parseFloat(vs[i].y) || 0, xj = parseFloat(vs[j].x) || 0, yj = parseFloat(vs[j].y) || 0;
    ...
    function isHoleInPoly(h, verts) {
        const vs = verts.filter(v => v.x !== "" && v.y !== "");
        const hx = parseFloat(h.x) || 0, hy = parseFloat(h.y) || 0;
        if (!ptInPoly(hx, hy, vs)) return false;
        const r = (parseFloat(h.diameter) || 0.5) / 2;
        for (let i = 0; i < vs.length; i++) {
            const j = (i + 1) % vs.length;
            if (distToSeg(hx, hy, parseFloat(vs[i].x) || 0, parseFloat(vs[i].y) || 0, parseFloat(vs[j].x) || 0, parseFloat(vs[j].y) || 0) < r)
                return false;
        }
        return true;
    }

    function calcCornerHolesPoly(polyVerts, margin, diameter) {
        const vs = polyVerts.filter(v => v.x !== "" && v.y !== "");
        if (vs.length < 3) return [];
        const cxP = vs.reduce((s, v) => s + (parseFloat(v.x) || 0), 0) / vs.length;
        const cyP = vs.reduce((s, v) => s + (parseFloat(v.y) || 0), 0) / vs.length;
        return vs.map((v, i) => {
            const n = vs.length, vx = parseFloat(v.x) || 0, vy = parseFloat(v.y) || 0;
            const px = parseFloat(vs[(i - 1 + n) % n].x) || 0, py = parseFloat(vs[(i - 1 + n) % n].y) || 0;
            const nx = parseFloat(vs[(i + 1) % n].x) || 0, ny = parseFloat(vs[(i + 1) % n].y) || 0;
            const d1 = Math.hypot(px - vx, py - vy), d2 = Math.hypot(nx - vx, ny - vy);
            if (d1 < 0.001 || d2 < 0.001) return { x: vx, y: vy, diameter };
            const u1x = (px - vx) / d1, u1y = (py - vy) / d1, u2x = (nx - vx) / d2, u2y = (ny - vy) / d2;
            const bx = u1x + u2x, by = u1y + u2y, bl = Math.hypot(bx, by);
            if (bl < 0.001) {
                const tl = Math.hypot(cxP - vx, cyP - vy);
                return tl < 0.001 ? { x: vx, y: vy, diameter } : { x: vx + (cxP - vx) / tl * margin, y: vy + (cyP - vy) / tl * margin, diameter };
            }
            const nbx = bx / bl, nby = by / bl;
            const dir = (nbx * (cxP - vx) + nby * (cyP - vy)) >= 0 ? 1 : -1;
            const sinHalf = Math.min(bl / 2, 1);
            const off = sinHalf > 0.05 ? margin / sinHalf : margin;
            return { x: vx + dir * nbx * off, y: vy + dir * nby * off, diameter };
        });
    }

    function getCustomBasePlateHoleCoordinates(bpConfig) {
        let raw = [];
        const shape = bpConfig.plateShape;
        const width = parseFloat(bpConfig.width) || 6.0;
        const height = parseFloat(bpConfig.height) || 6.0;
        if (shape === 'qiw_standard') {
            const type = bpConfig.qiwPlateType || 'QBP43';
            if (type === 'QBP54') {
                return [
                    { x: 0.5, y: 0.5, diameter: 0.5 },
                    { x: 4.5, y: 0.5, diameter: 0.5 },
                    { x: 4.5, y: 4.5, diameter: 0.5 },
                    { x: 0.5, y: 4.5, diameter: 0.5 }
                ];
            } else {
                return [
                    { x: 0.375, y: 0.375, diameter: 0.375 },
                    { x: 3.625, y: 0.375, diameter: 0.375 },
                    { x: 3.625, y: 3.625, diameter: 0.375 },
                    { x: 0.375, y: 3.625, diameter: 0.375 }
                ];
            }
        }
        ...
        const bounds = shape === 'rect' 
            ...
            : getPolyBounds(bpConfig.polyVerts);
            
        if (bpConfig.pattern === 'corners') {
            const margin = bpConfig.corners ? (parseFloat(bpConfig.corners.margin) || 1.0) : 1.0;
            const diameter = bpConfig.corners ? (parseFloat(bpConfig.corners.diameter) || 0.5) : 0.5;
            if (shape === 'rect') {
                raw = [
                    { x: margin, y: margin, diameter },
                    { x: width - margin, y: margin, diameter },
                    { x: width - margin, y: height - margin, diameter },
                    { x: margin, y: height - margin, diameter }
                ];
            } else {
                raw = calcCornerHolesPoly(bpConfig.polyVerts, margin, diameter);
            }
        } else if (bpConfig.pattern === 'grid') {
            const g = bpConfig.grid || {};
            const rows = Math.max(1, parseInt(g.rows) || 1);
            const cols = Math.max(1, parseInt(g.cols) || 1);
            const diameter = parseFloat(g.diameter) || 0.5;
            const marginX = parseFloat(g.marginX) || 1.0;
            const marginY = parseFloat(g.marginY) || 1.0;
            
            for (let r = 0; r < rows; r++) {
                for (let c = 0; c < cols; c++) {
                    const x = cols === 1 ? bounds.width / 2 : marginX + c * (bounds.width - 2 * marginX) / (cols - 1);
                    const y = rows === 1 ? bounds.height / 2 : marginY + r * (bounds.height - 2 * marginY) / (rows - 1);
                    raw.push({ x: bounds.minX + x, y: bounds.minY + y, diameter });
                }
            }
        } else if (bpConfig.pattern === 'custom') {
            const customList = bpConfig.custom || [];
            raw = customList.filter(h => h.x !== "" && h.y !== "" && parseFloat(h.diameter) > 0)
                .map(h => ({ x: parseFloat(h.x) || 0, y: parseFloat(h.y) || 0, diameter: parseFloat(h.diameter) || 0.5 }));
        }
        
        return raw.filter(h => {
            const r = h.diameter / 2;
            if (shape === 'rect') {
                return h.x - r >= 0 && h.x + r <= width && h.y - r >= 0 && h.y + r <= height;
            } else {
                return isHoleInPoly(h, bpConfig.polyVerts);
            }
        });
    }

    function getBasePlateCornerMargins(bpConfig) {
        const holes = getCustomBasePlateHoleCoordinates(bpConfig);
        if (!holes || holes.length === 0) {
            return { marginX: 0, marginY: 0, targetHole: null, bounds: { minX: 0, minY: 0 } };
        }
        const shape = bpConfig.plateShape;
        const width = parseFloat(bpConfig.width) || 6.0;
        const height = parseFloat(bpConfig.height) || 6.0;
        const bounds = shape === 'rect'
            ? { minX: 0, minY: 0, maxX: width, maxY: height, width, height }
            : (shape === 'qiw_standard' ? { minX: 0, minY: 0, maxX: (bpConfig.qiwPlateType === 'QBP54' ? 5.0 : 4.0), maxY: (bpConfig.qiwPlateType === 'QBP54' ? 5.0 : 4.0), width: (bpConfig.qiwPlateType === 'QBP54' ? 5.0 : 4.0), height: (bpConfig.qiwPlateType === 'QBP54' ? 5.0 : 4.0) } : getPolyBounds(bpConfig.polyVerts));

        // Find the hole closest to bottom-left corner (bounds.minX, bounds.minY)
        let targetHole = null;
        let minDist = Infinity;
        holes.forEach(h => {
            const dx = h.x - bounds.minX;
            const dy = h.y - bounds.minY;
        ...
        if (!targetHole) {
            return { marginX: 0, marginY: 0, targetHole: null, bounds };
        }

        const marginX = targetHole.x - bounds.minX;
        const marginY = targetHole.y - bounds.minY;
        return { marginX, marginY, targetHole, bounds };
    }

    function createCustomBasePlateModel(bpConfig) {
        if (!bpConfig) bpConfig = getDefaultBasePlateConfig();
        const model = { models: {}, paths: {} };
        
        if (bpConfig.plateShape === 'rect') {
            model.models.outer = new makerjs.models.Rectangle(parseFloat(bpConfig.width) || 6.0, parseFloat(bpConfig.height) || 6.0);
        } else if (bpConfig.plateShape === 'qiw_standard') {
            const w = bpConfig.qiwPlateType === 'QBP54' ? 5.0 : 4.0;
            const h = bpConfig.qiwPlateType === 'QBP54' ? 5.0 : 4.0;
            model.models.outer = new makerjs.models.Rectangle(w, h);
        } else {
            const pts = bpConfig.polyVerts.filter(v => v.x !== "" && v.y !== "").map(v => [parseFloat(v.x) || 0, parseFloat(v.y) || 0]);
            if (pts.length >= 3) {
                model.models.outer = { paths: {} };
                for (let i = 0; i < pts.length; i++) {
                    const p1 = pts[i];
                    const p2 = pts[(i + 1) % pts.length];
                    model.models.outer.paths['edge' + i] = new makerjs.paths.Line(p1, p2);
                }
            } else {
                model.models.outer = new makerjs.models.Rectangle(6.0, 6.0);
            }
        }
        
        model.models.holes = { models: {}, paths: {} };
        const holes = getCustomBasePlateHoleCoordinates(bpConfig);
        holes.forEach((h, idx) => {
            const origin = [h.x, h.y];
            const d = h.diameter;
            const r = d / 2;
            if (bpConfig.holeType === 'slotted' && (parseFloat(bpConfig.slotLength) || 1.0) > d) {
                const sl = parseFloat(bpConfig.slotLength) || 1.0;
                const sa = parseFloat(bpConfig.slotAngle) || 0;
                const slot = { paths: {} };
                const halfD = (sl - d) / 2;
                if (halfD > 0.01) {
                    slot.paths.line1 = new makerjs.paths.Line([-halfD, r], [halfD, r]);
                    slot.paths.line2 = new makerjs.paths.Line([-halfD, -r], [halfD, -r]);
                    slot.paths.cap1 = new makerjs.paths.Arc([halfD, 0], r, -90, 90);
                    slot.paths.cap2 = new makerjs.paths.Arc([-halfD, 0], r, 90, 270);
                } else {
                    slot.paths.circle = new makerjs.paths.Circle([0, 0], r);
                }
                makerjs.model.rotate(slot, sa, [0, 0]);
                slot.origin = origin;
                model.models.holes.models['hole_' + idx] = slot;
            } else {
                model.models.holes.paths['hole_' + idx] = new makerjs.paths.Circle(origin, r);
            }
        });
    ...
    function saveBasePlateInputs(bpConfig) {
        ...
        const thicknessInp = document.getElementById('inp-bp-thickness');
        if (thicknessInp) bpConfig.thickness = parseFloat(thicknessInp.value) || 0.5;
        const widthInp = document.getElementById('inp-bp-width');
        if (widthInp) bpConfig.width = parseFloat(widthInp.value) || 6.0;
        const heightInp = document.getElementById('inp-bp-height');
        if (heightInp) bpConfig.height = parseFloat(heightInp.value) || 6.0;
        
        const patternSelect = document.getElementById('inp-bp-pattern');
        if (patternSelect) bpConfig.pattern = patternSelect.value;
        const holeTypeSelect = document.getElementById('inp-bp-holeType');
        if (holeTypeSelect) bpConfig.holeType = holeTypeSelect.value;
        
        const slotLengthInp = document.getElementById('inp-bp-slotLength');
        if (slotLengthInp) bpConfig.slotLength = parseFloat(slotLengthInp.value) || 1.0;
        const slotAngleInp = document.getElementById('inp-bp-slotAngle');
        if (slotAngleInp) bpConfig.slotAngle = parseFloat(slotAngleInp.value) || 0.0;
        
        if (!bpConfig.corners) bpConfig.corners = { diameter: 0.5, margin: 1.0 };
        const cornersDiaInp = document.getElementById('inp-bp-corners-diameter');
        if (cornersDiaInp) bpConfig.corners.diameter = parseFloat(cornersDiaInp.value) || 0.5;
        const cornersMarginInp = document.getElementById('inp-bp-corners-margin');
        if (cornersMarginInp) bpConfig.corners.margin = parseFloat(cornersMarginInp.value) || 1.0;
        
        if (!bpConfig.grid) bpConfig.grid = { rows: 2, cols: 2, diameter: 0.5, marginX: 1.0, marginY: 1.0 };
        const gridRowsInp = document.getElementById('inp-bp-grid-rows');
        if (gridRowsInp) bpConfig.grid.rows = parseInt(gridRowsInp.value) || 2;
        const gridColsInp = document.getElementById('inp-bp-grid-cols');
        if (gridColsInp) bpConfig.grid.cols = parseInt(gridColsInp.value) || 2;
        const gridDiaInp = document.getElementById('inp-bp-grid-diameter');
        if (gridDiaInp) bpConfig.grid.diameter = parseFloat(gridDiaInp.value) || 0.5;
        const gridMarginXInp = document.getElementById('inp-bp-grid-marginX');
        if (gridMarginXInp) bpConfig.grid.marginX = parseFloat(gridMarginXInp.value) || 1.0;
        const gridMarginYInp = document.getElementById('inp-bp-grid-marginY');
        if (gridMarginYInp) bpConfig.grid.marginY = parseFloat(gridMarginYInp.value) || 1.0;
        
        const vertList = document.getElementById('bp-poly-vertices-list');
        if (vertList) {
            const rows = vertList.querySelectorAll('.bp-vertex-row');
            bpConfig.polyVerts = Array.from(rows).map((row, idx) => {
                const xVal = row.querySelector('.bp-v-x').value;
                const yVal = row.querySelector('.bp-v-y').value;
                return { id: idx + 1, x: xVal, y: yVal };
            });
        }
        
        const holeList = document.getElementById('bp-custom-holes-list');
        if (holeList) {
            const rows = holeList.querySelectorAll('.bp-hole-row');
            bpConfig.custom = Array.from(rows).map((row, idx) => {
                const xVal = row.querySelector('.bp-h-x').value;
                const yVal = row.querySelector('.bp-h-y').value;
                const dVal = row.querySelector('.bp-h-d').value;
                return { id: idx + 1, x: xVal, y: yVal, diameter: dVal };
            });
        }
    }
    ...
    function updateBasePlateCADPreview(bpConfig) {
        ...
        if (dimText) {
            if (bpConfig.plateShape === 'rect') {
                ...
                const vsCount = bpConfig.polyVerts.filter(v => v.x !== "" && v.y !== "").length;
                dimText.textContent = `Base Plate: CUSTOM ${vsCount}-VTX x ${bpConfig.thickness}" [Poly]`;
            }
        }
    ...
    function updateBasePlatePDFPreview(bpConfig) {
        ...
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(5);
        doc.text("0", 78, blockY + 7.5, { align: "center" });
        doc.text(today, 88.5, blockY + 7.5, { align: "center" });
        doc.text("FOR APPROVAL", 98, blockY + 7.5, { align: "left" });
        ...
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(6);
        doc.text("STANDARD PART", 205, blockY + 5.5, { align: "center" });
        
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(18);
        doc.text(displayMark, 205, blockY + 14.5, { align: "center" });
        
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(7.5);
        let stampDesc = "BALCONY RAIL EMBED";
        if (bpConfig.plateShape === 'qiw_standard' && bpConfig.qiwPlateType === 'QBP54') {
            stampDesc = "BALCONY RAIL EMBED PLATE";
        }
        doc.text(stampDesc, 205, blockY + 22, { align: "center" });
        
        // Right Stamp details block (width 25)
        let sheetDate = today;
        if (bpConfig.plateShape === 'qiw_standard') {
            if (bpConfig.qiwPlateType === 'QBP54') {
                sheetDate = "09/25/24";
            } else {
                sheetDate = "08/15/24";
            }
        ...
        doc.line(265, blockY + 9, 290, blockY + 9);
        doc.line(265, blockY + 18, 290, blockY + 18);
        
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(3.5);
        doc.text("DRAWN BY:", 266, blockY + 3);
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(6);
        doc.text("CDF", 277.5, blockY + 7, { align: "center" });
        
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(3.5);
        doc.text("CHECKED BY:", 266, blockY + 12);
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(6);
        doc.text("QIW", 277.5, blockY + 16, { align: "center" });
        
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(3.5);
        doc.text("DATE:", 266, blockY + 21);
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(6);
        ...
        if (panelObj) {
            ...
            if (isLoose) {
                ...
                const midPostCount = (panelObj.midPosts === 'default' || panelObj.midPosts === 'yes') 
                    ? Math.max(0, Math.ceil((panelObj.originalLength || panelObj.length) / 48) - 1) 
                    : ((panelObj.midPosts === 'custom' || panelObj.midPosts === 'custom_standard') ? (parseInt(panelObj.midPostCount) || 0) : 0);
                totalQty = (hasLeft ? 1 : 0) + (hasRight ? 1 : 0) + midPostCount;
            }
        }
        ...
        doc.setLineWidth(0.28);
        doc.setDrawColor(0, 0, 0);
        doc.rect(bomX, bomY, bomW, bomH, 'S');
        doc.setFillColor(235, 238, 242);
        doc.rect(bomX, bomY, bomW, 5, 'FD');
        doc.line(bomX, bomY + 5, bomX + bomW, bomY + 5);
        
        doc.line(bomX + 12, bomY, bomX + 12, bomY + bomH);
        doc.line(bomX + 20, bomY, bomX + 20, bomY + bomH);
        doc.line(bomX + 45, bomY, bomX + 45, bomY + bomH);
        
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(4.5);
        doc.setTextColor(0, 0, 0);
        doc.text("MARK", bomX + 6, bomY + 3.5, { align: "center" });
        doc.text("QTY", bomX + 16, bomY + 3.5, { align: "center" });
        doc.text("DESCRIPTION", bomX + 32.5, bomY + 3.5, { align: "center" });
        doc.text("GRADE", bomX + 50, bomY + 3.5, { align: "center" });
        
        let bpDesc = "";
        if (bpConfig.plateShape === 'rect') {
            bpDesc = `${formatFraction(bpConfig.width)}x${formatFraction(bpConfig.height)}x${formatFraction(bpConfig.thickness)} PL`;
        } else if (bpConfig.plateShape === 'qiw_standard') {
            bpDesc = bpConfig.qiwPlateType || 'QBP43';
        } else {
            const vsCount = bpConfig.polyVerts.filter(v => v.x !== "" && v.y !== "").length;
            bpDesc = `CUSTOM ${vsCount}-VTX x ${formatFraction(bpConfig.thickness)} PL`;
        }
        
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(4.5);
        doc.text(pieceMark, bomX + 6, bomY + 8.5, { align: "center" });
        doc.text(String(totalQty), bomX + 16, bomY + 8.5, { align: "center" });
        
        let dText = bpDesc;
        doc.setFontSize(4.5);
        let textW = doc.getTextWidth(dText);
        let dFontSize = 4.5;
        while (textW > 23 && dFontSize > 2.5) {
            dFontSize -= 0.1;
            doc.setFontSize(dFontSize);
            textW = doc.getTextWidth(dText);
        }
        doc.text(dText, bomX + 32.5, bomY + 8.5, { align: "center" });
        
        doc.setFontSize(4.5);
        doc.text("A36", bomX + 50, bomY + 8.5, { align: "center" });
        
        // Draw bottom-left circular mark stamp
        const circleX = 18;
        const circleY = blockY - 14;
        const circleR = 7.5;
        
        doc.setLineWidth(0.28);
        doc.setDrawColor(0, 0, 0);
        doc.circle(circleX, circleY, circleR, 'S');
        doc.line(circleX - circleR, circleY, circleX + circleR, circleY);
        
        doc.setFont('helvetica', 'normal');
        doc.setFontSize(8.5);
        doc.text("1", circleX, circleY - 1.5, { align: "center" });
        
        let circleMarkSize = 7.5;
        if (displayMark.length > 5) circleMarkSize = 5.5;
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(circleMarkSize);
        doc.text(displayMark, circleX, circleY + circleR - 2.5, { align: "center" });
        
        // Draw the REQ'D label next to it
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(10);
        let mkLabel = "BASEPLATE MK: " + displayMark;
        if (bpConfig.plateShape === 'qiw_standard' && bpConfig.qiwPlateType === 'QBP54') {
            mkLabel = "EMBED MK: " + displayMark;
        }
        const reqdText = "1 (" + totalQty + ") REQ'D - " + mkLabel;
        doc.text(reqdText, circleX + 11, circleY + 1.5);
        
        // Draw underline under reqdText
        const textWidth = doc.getTextWidth(reqdText);
        doc.line(circleX + 11, circleY + 3.0, circleX + 11 + textWidth, circleY + 3.0);
        ...
        pdfIframe.src = activePdfPreviewUrl;
    }

    function openBasePlatePartsModal() {
        balconyWizardState.customParts = balconyWizardState.customParts || [];
        
        let modal = document.getElementById('bp-parts-modal');
        if (!modal) {
            modal = document.createElement('div');
            modal.id = 'bp-parts-modal';
        ...
        const renderPartsList = () => {
            const partsList = balconyWizardState.customParts;
            const tbody = document.getElementById('bp-parts-tbody');
            if (!tbody) return;
            
            if (partsList.length === 0) {
                tbody.innerHTML = `
                    <tr>
                        <td colspan="5" style="text-align: center; color: #8c9ba5; padding: 15px; font-style: italic;">No parts added yet. Use the form below to add a part.</td>
                    </tr>
                `;
            } else {
                tbody.innerHTML = partsList.map((part, idx) => `
                    <tr style="border-bottom: 1px solid rgba(255,255,255,0.05);">
                        <td style="padding: 10px; color: #fff;">${part.quantity}</td>
                        <td style="padding: 10px; color: #fff; word-break: break-all;">${escapeHtml(part.description)}</td>
                        <td style="padding: 10px; color: #fff;">${escapeHtml(part.finish)}</td>
                        <td style="padding: 10px; color: #fff; word-break: break-all;">${escapeHtml(part.specialInstruction)}</td>
                        <td style="padding: 10px; text-align: center;">
                            <button class="btn-delete-part btn secondary" data-idx="${idx}" style="padding: 4px 8px; font-size: 10px; background: #ff4444; color: #fff; border: none; border-radius: 4px; cursor: pointer;">Delete</button>
                        </td>
                    </tr>
                `).join('');
                ...
                tbody.querySelectorAll('.btn-delete-part').forEach(btn => {
                    btn.addEventListener('click', (e) => {
                        ...
                        balconyWizardState.customParts.splice(index, 1);
                        renderPartsList();
                    });
                });
            }
        ...
        modal.innerHTML = `
            <div class="modal-content" style="max-width: 650px; width: 90%; background: #1a2329; border: 1px solid #2a3b45; border-radius: 12px; padding: 20px; color: #fff; display: flex; flex-direction: column; gap: 15px; max-height: 85vh; overflow-y: auto;">
                <h2 style="margin: 0; font-size: 1.25rem; font-weight: 700; color: #00d2ff; border-bottom: 1px solid #2a3b45; padding-bottom: 10px; display: flex; align-items: center; gap: 8px;">
                    <i data-lucide="component" style="width: 20px; height: 20px; color: #00ff88;"></i> Base Plate Parts Manager
                </h2>
                
                <div style="max-height: 250px; overflow-y: auto; border: 1px solid #2a3b45; border-radius: 6px; background: rgba(0,0,0,0.2);">
                    <table style="width: 100%; border-collapse: collapse; font-size: 12px; text-align: left;">
                        <thead>
                            <tr style="background: rgba(255,255,255,0.03); border-bottom: 1px solid #2a3b45;">
                                <th style="padding: 8px 10px; font-weight: 600; color: #8c9ba5;">Qty</th>
                                <th style="padding: 8px 10px; font-weight: 600; color: #8c9ba5;">Description</th>
                                <th style="padding: 8px 10px; font-weight: 600; color: #8c9ba5;">Finish</th>
                                <th style="padding: 8px 10px; font-weight: 600; color: #8c9ba5;">Special order instructions</th>
                                <th style="padding: 8px 10px; font-weight: 600; color: #8c9ba5; text-align: center; width: 60px;">Action</th>
                            </tr>
                        </thead>
                        <tbody id="bp-parts-tbody">
                            <!-- Populated dynamically -->
                        </tbody>
                    </table>
                </div>
                
                <form id="bp-add-part-form" onsubmit="return false;" style="display: flex; flex-direction: column; gap: 12px; border-top: 1px solid #2a3b45; padding-top: 15px;">
                    <h3 style="margin: 0; font-size: 0.95rem; color: #00ff88;">Add New Part</h3>
                    
                    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                        <div class="input-group">
                            <label style="display: block; font-size: 11px; margin-bottom: 4px; color: #8c9ba5;">Quantity *</label>
                            <input type="number" id="bp-part-qty" value="1" min="1" required style="width: 100%; padding: 8px; background: #111a1e; border: 1px solid #2a3b45; border-radius: 6px; color: #fff; box-sizing: border-box;">
                        </div>
                        <div class="input-group">
                            <label style="display: block; font-size: 11px; margin-bottom: 4px; color: #8c9ba5;">Finish *</label>
                            <input type="text" id="bp-part-finish" value="Powder Coat - Gloss Black" required style="width: 100%; padding: 8px; background: #111a1e; border: 1px solid #2a3b45; border-radius: 6px; color: #fff; box-sizing: border-box;">
                        </div>
                    </div>
                    
                    <div class="input-group">
                        <label style="display: block; font-size: 11px; margin-bottom: 4px; color: #8c9ba5;">Description *</label>
                        <input type="text" id="bp-part-desc" placeholder="e.g. Anchor Bolts 1/2&quot; x 6&quot;" required style="width: 100%; padding: 8px; background: #111a1e; border: 1px solid #2a3b45; border-radius: 6px; color: #fff; box-sizing: border-box;">
                    </div>
                    
                    <div class="input-group">
                        <label style="display: block; font-size: 11px; margin-bottom: 4px; color: #8c9ba5;">Special order instructions</label>
                        <textarea id="bp-part-special" rows="2" style="width: 100%; padding: 8px; background: #111a1e; border: 1px solid #2a3b45; border-radius: 6px; color: #fff; font-family: inherit; resize: vertical; box-sizing: border-box;"></textarea>
                    </div>
                    
                    <button type="button" id="btn-bp-add-part-submit" class="btn success" style="padding: 10px; font-weight: bold; background: #00ff88; color: #000; border: none; border-radius: 6px; cursor: pointer;">+ Add Part</button>
                </form>
                
                <div class="modal-actions" style="margin-top: 10px; padding-top: 15px; border-top: 1px solid #2a3b45; display: flex; justify-content: flex-end; gap: 12px;">
                    <button type="button" id="btn-bp-parts-close" class="btn secondary" style="padding: 8px 16px; background: #2a3b45; color: #fff; border: none; border-radius: 6px; cursor: pointer;">Close & Apply</button>
                </div>
            </div>
        `;
        ...
        document.getElementById('btn-bp-add-part-submit').addEventListener('click', () => {
            const qtyInput = document.getElementById('bp-part-qty');
            const finishInput = document.getElementById('bp-part-finish');
            const descInput = document.getElementById('bp-part-desc');
            const specialInput = document.getElementById('bp-part-special');
            ...
            if (!desc || !finish) {
                alert("Please fill in Description and Finish");
                return;
            }
            
            balconyWizardState.customParts.push({
                quantity: qty,
                description: desc,
                finish: finish,
        ...
        document.getElementById('btn-bp-parts-close').addEventListener('click', () => {
    ...
    function renderBasePlateEditor() {
        ...
        const advFeaturesBtn = document.getElementById('btn-advance-features');
        if (togglePdfBtn) togglePdfBtn.classList.add('hidden');
        if (advFeaturesBtn) advFeaturesBtn.classList.add('hidden');
        
        // Auto-enable pan mode when entering Base Plate Designer
        if (!panModeActive && togglePanBtn) {
            togglePanBtn.click();
        }
        
        // Reset zoom and pan to fit by default once upon entering
        currentZoom = 0.8;
        currentPanX = 0;
        currentPanY = 0;
        applyZoom();
        ...
        if (viewTitle) {
            viewTitle.innerHTML = `
                <div style="display: flex; gap: 8px; align-items: center;">
                    <button id="btn-bp-tab-cad" class="btn ${!pdfPreviewModeActive ? 'primary' : 'secondary'}" style="padding: 4px 10px; font-size: 11px; font-weight: bold; cursor: pointer;">CAD Preview</button>
                    <button id="btn-bp-tab-pdf" class="btn ${pdfPreviewModeActive ? 'primary' : 'secondary'}" style="padding: 4px 10px; font-size: 11px; font-weight: bold; cursor: pointer;">PDF Preview</button>
                </div>
            `;
            document.getElementById('btn-bp-tab-cad').addEventListener('click', () => {
                pdfPreviewModeActive = false;
                document.getElementById('btn-bp-tab-cad').className = "btn primary";
                document.getElementById('btn-bp-tab-pdf').className = "btn secondary";
                svgContainer.classList.remove('hidden');
                pdfIframe.classList.add('hidden');
                renderCurrentCAD();
            });
            document.getElementById('btn-bp-tab-pdf').addEventListener('click', () => {
                pdfPreviewModeActive = true;
                document.getElementById('btn-bp-tab-cad').className = "btn secondary";
                document.getElementById('btn-bp-tab-pdf').className = "btn primary";
                svgContainer.classList.add('hidden');
                pdfIframe.classList.remove('hidden');
                updatePdfPreview();
            });
        ...
        let html = `
        <div class="bp-editor-container" style="display: flex; flex-direction: column; gap: 15px; font-family: 'Inter', sans-serif; color: var(--text-color);">
            <div style="display: flex; align-items: center; justify-content: space-between; border-bottom: 1px solid var(--border-color); padding-bottom: 10px; margin-bottom: 5px;">
            ...
            <div class="input-group">
                ...
                <select id="inp-bp-plateShape">
                    <option value="rect" ${bpConfig.plateShape === 'rect' ? 'selected' : ''}>Rectangular</option>
                    <option value="poly" ${bpConfig.plateShape === 'poly' ? 'selected' : ''}>Custom Polygon</option>
                    <option value="qiw_standard" ${bpConfig.plateShape === 'qiw_standard' ? 'selected' : ''}>QIW Standard Plates</option>
                </select>
            </div>

            <div id="grp-bp-qiw-options" class="input-group" style="display: ${bpConfig.plateShape === 'qiw_standard' ? 'block' : 'none'};">
                <label>Standard Plate Type</label>
                <select id="inp-bp-qiwPlateType">
                    <option value="QBP43" ${bpConfig.qiwPlateType === 'QBP43' ? 'selected' : ''}>QBP43 (4"x4"x3/16")</option>
                    <option value="QBP54" ${bpConfig.qiwPlateType === 'QBP54' ? 'selected' : ''}>QBP54 (5"x5"x1/4")</option>
                </select>
            </div>
            
            <div id="grp-bp-thickness-container" class="input-group" style="display: ${bpConfig.plateShape !== 'qiw_standard' ? 'block' : 'none'};">
                <label>Plate Thickness (in)</label>
                <input type="number" id="inp-bp-thickness" value="${bpConfig.thickness || 0.5}" step="0.0625" min="0.125">
            </div>
            
            <div id="grp-bp-rect-dims" class="inputs-subgrid" style="display: ${bpConfig.plateShape === 'rect' ? 'grid' : 'none'}; grid-template-columns: 1fr 1fr; gap: 10px;">
                <div class="input-group">
                    <label>Width (in)</label>
                    <input type="number" id="inp-bp-width" value="${bpConfig.width || 6.0}" step="0.0625" min="1">
                </div>
                <div class="input-group">
                    <label>Height/Length (in)</label>
                    <input type="number" id="inp-bp-height" value="${bpConfig.height || 6.0}" step="0.0625" min="1">
                </div>
            </div>
            
            <div id="grp-bp-poly-dims" style="display: ${bpConfig.plateShape === 'poly' ? 'flex' : 'none'}; flex-direction: column; gap: 8px;">
                <label style="font-size: 11px; font-weight: bold; color: var(--text-dim);">Polygon Vertices (0,0 is bottom-left)</label>
                <div style="display: grid; grid-template-columns: 1fr 1fr 30px; gap: 6px; font-size: 10px; color: var(--text-dim); font-weight: bold; padding-left: 20px;">
                    <span>X (in)</span>
                    <span>Y (in)</span>
                    <span></span>
                </div>
                <div id="bp-poly-vertices-list" style="display: flex; flex-direction: column; gap: 6px;"></div>
                <button id="btn-bp-add-vertex" class="btn secondary" style="padding: 6px; font-size: 11px; cursor: pointer; border-style: dashed; width: 100%;">+ Add Vertex</button>
            </div>
            
            <div id="grp-bp-pattern-container" class="input-group" style="display: ${bpConfig.plateShape !== 'qiw_standard' ? 'block' : 'none'}; margin-top: 5px;">
                <label>Hole Pattern</label>
                <select id="inp-bp-pattern">
                    <option value="corners" ${bpConfig.pattern === 'corners' ? 'selected' : ''}>Standard</option>
                    <option value="grid" ${bpConfig.pattern === 'grid' ? 'selected' : ''}>With Coordinates</option>
                    <option value="custom" ${bpConfig.pattern === 'custom' ? 'selected' : ''}>Custom</option>
                </select>
            </div>
            
            <div id="grp-bp-pattern-corners" class="inputs-subgrid" style="display: ${bpConfig.plateShape !== 'qiw_standard' && bpConfig.pattern === 'corners' ? 'grid' : 'none'}; grid-template-columns: 1fr 1fr; gap: 10px;">
                <div class="input-group">
                    <label>Hole Diameter (in)</label>
                    <input type="number" id="inp-bp-corners-diameter" value="${bpConfig.corners.diameter || 0.5}" step="0.0625" min="0.125">
                </div>
                <div class="input-group">
                    <label>Corner Margin (in)</label>
                    <input type="number" id="inp-bp-corners-margin" value="${bpConfig.corners.margin || 1.0}" step="0.0625" min="0">
                </div>
            </div>
            
            <div id="grp-bp-pattern-grid" class="inputs-subgrid" style="display: ${bpConfig.plateShape !== 'qiw_standard' && bpConfig.pattern === 'grid' ? 'grid' : 'none'}; grid-template-columns: 1fr 1fr; gap: 10px;">
                <div class="input-group">
                    <label>Rows</label>
                    <input type="number" id="inp-bp-grid-rows" value="${bpConfig.grid.rows || 2}" min="1" step="1">
                </div>
                <div class="input-group">
                    <label>Columns</label>
                    <input type="number" id="inp-bp-grid-cols" value="${bpConfig.grid.cols || 2}" min="1" step="1">
                </div>
                <div class="input-group">
                    <label>Hole Diameter (in)</label>
                    <input type="number" id="inp-bp-grid-diameter" value="${bpConfig.grid.diameter || 0.5}" step="0.0625" min="0.125">
                </div>
                <div class="input-group">
                    <label>Margin X (in)</label>
                    <input type="number" id="inp-bp-grid-marginX" value="${bpConfig.grid.marginX || 1.0}" step="0.0625" min="0">
                </div>
                <div class="input-group" style="grid-column: span 2;">
                    <label>Margin Y (in)</label>
                    <input type="number" id="inp-bp-grid-marginY" value="${bpConfig.grid.marginY || 1.0}" step="0.0625" min="0">
                </div>
            </div>
            
            <div id="grp-bp-pattern-custom" style="display: ${bpConfig.plateShape !== 'qiw_standard' && bpConfig.pattern === 'custom' ? 'flex' : 'none'}; flex-direction: column; gap: 8px;">
                <label style="font-size: 11px; font-weight: bold; color: var(--text-dim);">Custom Hole Coordinates</label>
                <div style="display: grid; grid-template-columns: 1fr 1fr 1fr 30px; gap: 6px; font-size: 10px; color: var(--text-dim); font-weight: bold; padding-left: 20px;">
                    <span>X (in)</span>
                    <span>Y (in)</span>
                    <span>Ã˜ (in)</span>
                    <span></span>
                </div>
                <div id="bp-custom-holes-list" style="display: flex; flex-direction: column; gap: 6px;"></div>
                <button id="btn-bp-add-hole" class="btn secondary" style="padding: 6px; font-size: 11px; cursor: pointer; border-style: dashed; width: 100%;">+ Add Hole</button>
            </div>
            
            <div id="grp-bp-holeType-container" class="input-group" style="display: ${bpConfig.plateShape !== 'qiw_standard' ? 'block' : 'none'}; margin-top: 5px;">
                <label>Hole Type</label>
                <select id="inp-bp-holeType">
                    <option value="round" ${bpConfig.holeType === 'round' ? 'selected' : ''}>Round</option>
                    <option value="slotted" ${bpConfig.holeType === 'slotted' ? 'selected' : ''}>Slotted</option>
                </select>
            </div>
            
            <div id="grp-bp-slot-options" class="inputs-subgrid" style="display: ${bpConfig.plateShape !== 'qiw_standard' && bpConfig.holeType === 'slotted' ? 'grid' : 'none'}; grid-template-columns: 1fr 1fr; gap: 10px;">
                <div class="input-group">
                    <label>Slot Length (in)</label>
                    <input type="number" id="inp-bp-slotLength" value="${bpConfig.slotLength || 1.0}" step="0.0625" min="0.125">
                </div>
                <div class="input-group">
                    <label>Slot Angle (Â°)</label>
                    <input type="number" id="inp-bp-slotAngle" value="${bpConfig.slotAngle || 0.0}" min="0" max="360" step="1">
                </div>
            </div>
        ...
        document.getElementById('btn-bp-manage-parts').addEventListener('click', openBasePlatePartsModal);
        ...
        const renderVertexRows = () => {
            const list = document.getElementById('bp-poly-vertices-list');
            if (!list) return;
            list.innerHTML = bpConfig.polyVerts.map((v, idx) => `
                <div class="bp-vertex-row" style="display: grid; grid-template-columns: 1fr 1fr 30px; gap: 6px; align-items: center;">
                    <span style="font-size: 10px; color: var(--text-dim); text-align: right; padding-right: 5px;">${idx + 1}</span>
                    <input type="number" class="bp-v-x" value="${v.x}" step="0.0625" style="padding: 4px 6px; font-size: 11px;">
                    <input type="number" class="bp-v-y" value="${v.y}" step="0.0625" style="padding: 4px 6px; font-size: 11px;">
                    <button class="btn-remove-vertex" data-idx="${idx}" style="background: none; border: none; color: #ff6b6b; font-size: 16px; cursor: pointer;">&times;</button>
                </div>
            `).join('');
            
            list.querySelectorAll('.btn-remove-vertex').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    const idx = parseInt(e.currentTarget.getAttribute('data-idx'));
                    bpConfig.polyVerts.splice(idx, 1);
                    renderVertexRows();
        ...
        const renderHoleRows = () => {
            const list = document.getElementById('bp-custom-holes-list');
            if (!list) return;
            list.innerHTML = bpConfig.custom.map((h, idx) => `
                <div class="bp-hole-row" style="display: grid; grid-template-columns: 1fr 1fr 1fr 30px; gap: 6px; align-items: center;">
                    <span style="font-size: 10px; color: var(--text-dim); text-align: right; padding-right: 5px;">${idx + 1}</span>
                    <input type="number" class="bp-h-x" value="${h.x}" step="0.0625" style="padding: 4px 6px; font-size: 11px;">
                    <input type="number" class="bp-h-y" value="${h.y}" step="0.0625" style="padding: 4px 6px; font-size: 11px;">
                    <input type="number" class="bp-h-d" value="${h.diameter}" step="0.0625" style="padding: 4px 6px; font-size: 11px;">
                    <button class="btn-remove-hole-row" data-idx="${idx}" style="background: none; border: none; color: #ff6b6b; font-size: 16px; cursor: pointer;">&times;</button>
                </div>
            `).join('');
            
            list.querySelectorAll('.btn-remove-hole-row').forEach(btn => {
                btn.addEventListener('click', (e) => {
                    const idx = parseInt(e.currentTarget.getAttribute('data-idx'));
                    bpConfig.custom.splice(idx, 1);
                    renderHoleRows();
                    saveInputsAndRefresh();
                });
            });
        ...
        document.getElementById('inp-bp-plateShape').addEventListener('change', (e) => {
            const val = e.target.value;
            const isQiw = val === 'qiw_standard';
            const isRect = val === 'rect';
            const isPoly = val === 'poly';
            
            document.getElementById('grp-bp-qiw-options').style.display = isQiw ? 'block' : 'none';
            document.getElementById('grp-bp-thickness-container').style.display = isQiw ? 'none' : 'block';
            document.getElementById('grp-bp-rect-dims').style.display = isRect ? 'grid' : 'none';
            document.getElementById('grp-bp-poly-dims').style.display = isPoly ? 'flex' : 'none';
            document.getElementById('grp-bp-pattern-container').style.display = isQiw ? 'none' : 'block';
            document.getElementById('grp-bp-holeType-container').style.display = isQiw ? 'none' : 'block';
            
            if (isQiw) {
                document.getElementById('grp-bp-pattern-corners').style.display = 'none';
                document.getElementById('grp-bp-pattern-grid').style.display = 'none';
                document.getElementById('grp-bp-pattern-custom').style.display = 'none';
                document.getElementById('grp-bp-slot-options').style.display = 'none';
            } else {
                const pat = document.getElementById('inp-bp-pattern').value;
                document.getElementById('grp-bp-pattern-corners').style.display = pat === 'corners' ? 'grid' : 'none';
                document.getElementById('grp-bp-pattern-grid').style.display = pat === 'grid' ? 'grid' : 'none';
                document.getElementById('grp-bp-pattern-custom').style.display = pat === 'custom' ? 'flex' : 'none';
                
                const isSlotted = document.getElementById('inp-bp-holeType').value === 'slotted';
                document.getElementById('grp-bp-slot-options').style.display = isSlotted ? 'grid' : 'none';
            }
            saveInputsAndRefresh();
        });
        
        const qiwSel = document.getElementById('inp-bp-qiwPlateType');
        if (qiwSel) {
            qiwSel.addEventListener('change', saveInputsAndRefresh);
        }
        
        document.getElementById('inp-bp-pattern').addEventListener('change', (e) => {
        ...
        document.getElementById('inp-bp-holeType').addEventListener('change', (e) => {
            const isSlotted = e.target.value === 'slotted';
            document.getElementById('grp-bp-slot-options').style.display = isSlotted ? 'grid' : 'none';
            saveInputsAndRefresh();
        });
        
        document.getElementById('btn-bp-add-vertex').addEventListener('click', () => {
            bpConfig.polyVerts.push({ id: bpConfig.polyVerts.length + 1, x: "0", y: "0" });
            renderVertexRows();
            saveInputsAndRefresh();
        });
        
        document.getElementById('btn-bp-add-hole').addEventListener('click', () => {
            bpConfig.custom.push({ id: bpConfig.custom.length + 1, x: "0", y: "0", diameter: "0.5" });
            renderHoleRows();
            saveInputsAndRefresh();
        });
        ...
        document.getElementById('btn-bp-close').addEventListener('click', () => {
            ...
            // Turn off pan mode on exiting
            if (panModeActive && togglePanBtn) {
                togglePanBtn.click();
            }
            
            if (togglePdfBtn) togglePdfBtn.classList.remove('hidden');
            if (togglePanBtn) togglePanBtn.classList.remove('hidden');
            if (advFeaturesBtn) advFeaturesBtn.classList.remove('hidden');
